"""
Somehow hacky but working log management, which allows to
unify both Uvicorn and application logs.
"""
...
class CustomFormatter(ColourizedFormatter):
    ...
    def formatMessage(self, record: logging.LogRecord) -> str:
        ...
        # align short names
        recordcopy.name = f"{recordcopy.name:<12}"
...
def get_uvicorn_logging_config() -> Dict[str, Any]:
    ...
    formatters["access"]["fmt"] = (
        LOG_PREFIX + '%(request)s %(client_addr)s - "%(request_line)s" %(status_code)s'
    )
